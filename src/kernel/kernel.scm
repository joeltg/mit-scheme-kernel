...
(define ((make-handler iopub-socket get-session signature-scheme key) socket env)
  (let ((blobs (reverse (receive-messages socket))))
    (let ((identities (take-while (lambda (x) (not (bytevector=? x delimiter))) blobs))
          (rest (drop-while (lambda (x) (not (bytevector=? x delimiter))) blobs)))
      (let ((deli (car rest))
            (hmac (cadr rest))
            (json (map vector-ref-0 (map json-decode (map utf8->string (cddr rest))))))
        (assert (bytevector=? deli delimiter))
        (let ((identity (car identities)) ; Use the first identity as the key
